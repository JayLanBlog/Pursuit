#include "console.h"
/*
	#include <util/time_h.h>
#include <util/imgui_help.h>
#include <util/obf.h>
*/
...
void Console::render() {
	...
	ImGui::Begin("Console");
	ImGuiHelper::drawTabHorizontally(obf("Styles"), ImVec2(ImGuiHelper::getWidth(), 60), types, selectedTypeTab);
	...
	if (ImGui::InputText(obf("Input").c_str(), inputBuf, sizeof(inputBuf), input_text_flags)) {
		...
		else if (s.compare(obf("sdcfg")) == 0);
		else if (s.compare(obf("ldcfg")) == 0);
		reclaim_focus = true;
	}
